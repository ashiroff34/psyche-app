...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  // Enneagram type-in-relationships pages — all 9 types
  const relationshipPages: MetadataRoute.Sitemap = ENNEAGRAM_TYPES.map((t) => ({
    url: `${BASE}/enneagram/type-${t}/relationships`,
    lastModified: now,
    changeFrequency: "monthly" as const,
    priority: 0.85,
  }));
  ...
  // Same-type compatibility pages — all 9 same-type pairs
  const sameTypeCompatibilityPages: MetadataRoute.Sitemap = ENNEAGRAM_TYPES.map((t) => ({
    url: `${BASE}/compatibility/enneagram-${t}/enneagram-${t}`,
    lastModified: now,
    changeFrequency: "monthly" as const,
    priority: 0.85,
  }));

  // Famous examples pages — all 9 types
  const famousPages: MetadataRoute.Sitemap = ENNEAGRAM_TYPES.map((t) => ({
    url: `${BASE}/enneagram/type-${t}/famous`,
    lastModified: now,
    changeFrequency: "monthly" as const,
    priority: 0.85,
  }));

  return [...core, ...typePages, ...unitPages, ...assessmentPages, ...seoEnneagramPages, ...subtypePages, ...cognitiveFunctionPages, ...compatibilityPages, ...enneagramTheoryPages, ...wingPages, ...relationshipPages, ...workPages, ...sameTypeCompatibilityPages, ...famousPages];
}
