...
function getTriad(t: EnneagramType): string {
  ...
  if (BODY_TYPES.includes(t)) return "body";
  return "body";
}
...
function generateContent(mbti: MBTIType, ennType: EnneagramType): CombinationContent {
  ...
  const howMbtiShapes = resonanceMap[domCat]?.[ennType] ??
    `${domFull} (${dom}) is the ${mbti}'s dominant function — the mode through which all experience is first processed. For the ${ennName}, this means that the type's core motivation is channeled through ${FUNCTION_BRIEF[dom]}. The cognitive style shapes not what the ${mbti} wants but how that wanting is organized and pursued.`;
...
function generateGrowthPath(
  ...
  const triadGrowthMap: Record<string, string> = {
    ...
    head: `The head triad's growth involves developing genuine trust — trust in one's own resources, in others, or in the larger pattern of events — rather than relying on mental activity to manufacture a sense of security. For the ${mbti}, ${dom} is both the strength and the potential trap: the cognitive capacity that makes them adept at analysis can also keep them circling in their heads rather than arriving at grounded presence.`,
  ...
  return `Integration for the ${mbti} ${ennName} moves toward ${integrationName} (Type ${integrationTarget}) on the Enneagram level. ${triadGrowthMap[triad]} The practical growth path combines the Enneagram's depth work on motivation with the cognitive type's own development work — specifically, developing the inferior function (${COGNITIVE_STACKS[mbti][3]}, ${FUNCTION_NAMES[COGNITIVE_STACKS[mbti][3]]}) as a pathway to wholeness. Both frameworks point toward the same destination: a person who can access the full range of their psychological resources rather than living exclusively within one well-worn groove.`;
}

// ── Slug parsing ──────────────────────────────────────────────────────────────
function parseSlug(slug: string): { mbti: MBTIType; ennType: EnneagramType } | null {
  // Expected format: "intj-enneagram-4"
  const match = slug.match(/^([a-z]{4})-enneagram-([1-9])$/);
  if (!match) return null;
  const mbti = match[1].toUpperCase() as MBTIType;
  const ennType = parseInt(match[2], 10) as EnneagramType;
  if (!MBTI_TYPES.includes(mbti) || !ENNEAGRAM_TYPES.includes(ennType)) return null;
  return { mbti, ennType };
}
...
export function generateStaticParams(): { slug: string }[] {
  ...
  for (const mbti of MBTI_TYPES) {
    for (const ennType of ENNEAGRAM_TYPES) {
      params.push({ slug: `${mbti.toLowerCase()}-enneagram-${ennType}` });
    }
  }
  return params;
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const { mbti, ennType } = parsed;
  const mbtiName = MBTI_NAMES[mbti];
  const ennName = TYPE_NAMES[ennType];
  const url = `${BASE}/personality/${slug}`;

  return {
    title: `${mbti} Enneagram ${ennType} — ${mbtiName} ${ennName}: Traits, Growth & Identity | Thyself`,
    description: `How the ${mbti} cognitive type and Enneagram ${ennType} (${ennName}) interact. Explore the ${mbti} ${ennName} cognitive pattern, growth path, common mistypes, and identity structure. Grounded in Jungian and Enneagram theory.`,
    alternates: { canonical: url },
    openGraph: {
      title: `${mbti} Enneagram ${ennType}: ${mbtiName} ${ennName}`,
      description: `How the ${mbti} cognitive type and Enneagram ${ennType} (${ennName}) interact.`,
      url,
      type: "article",
      siteName: "Thyself",
    },
...
export default async function CrossoverPage({ params }: PageProps) {
  ...
  const sameTypeDifferentEnn = ENNEAGRAM_TYPES
    .filter((t) => t !== ennType)
    .slice(0, 4)
    .map((t) => ({ type: t, name: TYPE_NAMES[t], slug: `${mbti.toLowerCase()}-enneagram-${t}` }));

  const sameEnnDifferentMbti: { mbti: MBTIType; name: string; slug: string }[] = MBTI_TYPES
    .filter((m) => m !== mbti)
    .slice(0, 4)
    .map((m) => ({ mbti: m, name: MBTI_NAMES[m], slug: `${m.toLowerCase()}-enneagram-${ennType}` }));
    ...
    <>
      ...
      <main className="min-h-screen" style={{ backgroundColor: "#0f0a1e", color: "rgba(255,255,255,0.92)" }}>
        ...
        <section className="px-6 py-16" style={{ background: `linear-gradient(135deg, ${mbtiColor} 0%, ${ennColor} 100%)` }}>
          <div className="mx-auto max-w-3xl">
            ...
            <div className="mb-6 flex flex-wrap gap-3">
              <span className="rounded-full px-4 py-1.5 text-sm font-semibold" style={{ backgroundColor: "rgba(255,255,255,0.2)", color: "rgba(255,255,255,0.95)" }}>
                {mbti} — {mbtiName}
              </span>
              <span className="rounded-full px-4 py-1.5 text-sm font-semibold" style={{ backgroundColor: ennColor, color: "#fff" }}>
            ...
            <div className="flex flex-wrap gap-2">
              {stack.map((fn, i) => {
                ...
                return (
                  ...
                  >
                    <span className="text-sm font-bold" style={{ color: i === 0 ? ennColor : "rgba(255,255,255,0.75)" }}>
                      {fn}
                    </span>
        ...
        <div className="mx-auto max-w-3xl space-y-14 px-6 py-14">
          ...
          <section className="rounded-xl p-6" style={{ backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)" }}>
            <h2 className="mb-4 text-2xl font-bold" style={{ color: "rgba(255,255,255,0.95)" }}>
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-2">
              {sameTypeDifferentEnn.map(({ type: t, name, slug: s }) => (
                ...
                >
                  {mbti} Enneagram {t}
                  <span className="ml-1 text-xs" style={{ color: "rgba(255,255,255,0.45)" }}>— {name}</span>
                </Link>
              ))}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-2">
              {sameEnnDifferentMbti.map(({ mbti: m, name, slug: s }) => (
                ...
                >
                  {m} Enneagram {ennType}
                  <span className="ml-1 text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>— {name}</span>
                </Link>
              ))}
            </div>
          </section>
          ...
          <div className="text-center">
            <Link href="/personality" className="text-sm transition-opacity hover:opacity-80" style={{ color: "rgba(255,255,255,0.4)" }}>
